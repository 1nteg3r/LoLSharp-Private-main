...
public:
	...
	PredictionInput Q = PredictionInput({ 1510, 0.7f,95.f,FLT_MAX, false, SkillshotType::SkillshotLine });
	...
	void castingQ()
	{
		...
		{
			Q.Range = MIN((750 + 500 * (Engine::TickCount() - qTick) / 1000), 1465.f);
		}

		auto qBuff = me->GetBuffManager()->GetBuffCacheByNameHash(FNV("XerathArcanopulseChargeUp"));
		...
		if (qBuff.count == 0)
		{
 		...
 		/*if (IsKeyDown(CheckKey(_Q)) == true && !IsReady(_Q))
		{
			Q.Range = 750;
	...
	void startQ()
	{
		...
		if (IsReady(_Q) && caststate[0] && chargeQXerath == false && start == true)
		{
			KeyDown(CheckKey(_Q));
		}
	...
	void Tick()
	{
		...
		if (orbwalker->mode & ScriptMode::Combo)
		{
