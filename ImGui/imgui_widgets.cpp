...
bool ImGui::Checkbox(const char* label, bool* v)
{
	...
	ImU32 color = GetColorU32(ImVec4(*v ? (hovered ? colors[ImGuiCol_ButtonHovered] : colors[ImGuiCol_CheckMark]) : (hovered ? colors[ImGuiCol_FrameBgHovered] : colors[ImGuiCol_FrameBg])));

	float radius = 8;
	float width = radius * 1.72;
	float btn_radius = radius * 0.80f;

	ImVec2 center = { check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y + radius * 1.5f };

	//window->DrawList->AddRectFilled(check_bb.Min+ImVec2(pad,pad), check_bb.Max-ImVec2(pad,pad), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);


	window->DrawList->AddCircleFilled({ center.x - width / 2, center.y }, radius - 3, color, 12);

	window->DrawList->AddCircleFilled({ center.x + width / 2, center.y }, radius - 3, color, 12);

	window->DrawList->AddRectFilled({ center.x - width / 2, center.y - radius + 3 }
	, { center.x + width / 2, center.y + radius-3}, color);

	window->DrawList->AddCircleFilled({ center.x + (*v ? width : -width) / 2, center.y }, btn_radius, 0xFFFFFFFF, 12);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay, const ImU32& bg_col, const ImU32& fg_col1)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return ;
	...
	if (fraction == 1.0f)
	{
		window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Max.x, bb.Max.y), ImGui::GetColorU32({ 83 / 255.0f, 166 / 255.0f, 83 / 255.0f, 255 / 255.0f }));
		return;
	}
	window->DrawList->AddRectFilled(bb.Min, ImVec2(pos.x + circleStart, bb.Max.y), bg_col);
	window->DrawList->AddRectFilled(bb.Min, ImVec2(pos.x + circleStart * fraction, bb.Max.y), fg_col1);
 ...
 //   if (!ItemAdd(bb, 0))
 //       return;
	//ImU32 fg_col = fg_col1;

	//if (fraction == 1.f)
	//	fg_col = ImGui::GetColorU32({ 83 / 255.0f, 166 / 255.0f, 83 / 255.0f, 255 / 255.0f });
 //   // Render
 //   fraction = ImSaturate(fraction);
 //   RenderFrame(bb.Min, bb.Max, bg_col, true, style.FrameRounding);
 //   bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
 //   const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
 //   RenderRectFilledRangeH(window->DrawList, bb, fg_col, 0.0f, fraction, style.FrameRounding);
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(60 + style.ItemInnerSpacing.x, 0.0f), window->DC.CursorPos + size);
