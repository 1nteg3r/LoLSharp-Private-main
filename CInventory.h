...
public:
	...
	kItemID ItemID(int slot)
	{
		return (kItemID)RPM<int>(RPM<uint32_t>(RPM<uint32_t>(RPM<uint32_t>(this + oItemSlotInst + 0x4 * slot) + oItemInfo) + oItemData) + oItemSlotID);
	}

	int Stack(int slot)
	{
		return RPM<int8_t>(RPM<uint32_t>(RPM<uint32_t>(this + 0x18 + 0x4 * slot) + 0xC) + 0x24);
	}
