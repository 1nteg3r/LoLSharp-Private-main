...
public:
	...
	bool ShouldWait()
	{
		...
		for (auto minion : minionListAA)
		{
			if (LaneClearHealthPrediction(minion,
				(Engine::GetAttackDelay() * 1000 * LaneClearWaitTimeMod) + (AttackCastDelay() * 1000) + BrainFarmInt + Engine::GetPing() / 2 + 1000 * 500 / speed
	...
	{
		float attackCalc = (Engine::GetAttackDelay() * 1000 + (me->IsMelee() ? AttackCastDelay() * 1000 : AttackCastDelay() * 1000 +
	...
	void DetectAutoAttacksAndSpells()
	{
		...
		if (ActiveAttacks.size() > 0)
		{
			...
			for (it = ActiveAttacks.begin(); it != ActiveAttacks.end(); it++)
			{
				auto id = it->first;
				auto data = it->second;
				if (data.startTime < Engine::GameTimeTickCount() - 3000)
				{
	...
	float GetHealthPrediction(CObject* unit, float time, float delay = 0)
	{
		...
		for (it = ActiveAttacks.begin(); it != ActiveAttacks.end(); it++)
		{
			...
			{
				auto source = (CObject*)attack.source;
				float landTime = attack.startTime + attack.Delay
	...
	CObject* GetTarget()
	{
		...
		auto firstT = (AttackCastDelay() * 1000) + BrainFarmInt + Engine::GetPing() / 2;
		...
		if (mode & ScriptMode::LaneClear || mode & ScriptMode::Harass || mode & ScriptMode::LastHit)
		{
			...
			sort(LastHitList.begin(), LastHitList.end(), [&](CObject* minion, CObject* minion2) {
				return GetHealthPrediction(minion, 1500) > GetHealthPrediction(minion2, 1500);
			});
			sort(LastHitList.begin(), LastHitList.end(), [&](CObject* minion, CObject* minion2) {
			...
			{
				auto t = firstT + 1000 * max(0, me->ServerPosition().Distance(minion->ServerPosition()) - me->BoundingRadius()) / projectileSpeed;
		...
		if (mode & ScriptMode::LaneClear)
		{
			if (!ShouldWait())
			{
				auto firstT2 = (Engine::GetAttackDelay() * 1000 * LaneClearWaitTimeMod) + (AttackCastDelay() * 1000) + BrainFarmInt + Engine::GetPing() / 2;
				...
				{
					auto t = firstT2 + 1000 * max(0, me->ServerPosition().Distance(minion->ServerPosition()) - me->BoundingRadius()) / projectileSpeed;
