local enemylib = module.load("int", "Library/util")
...
function CanMove(unit, delay)
    for i, buff in pairs(unit.buff) do
		if buff.stacks > 0 and buff.startTime >= delay then
...
function CheckMinionCollision(unit, Position, delay, radius, range, colison, argsColision, speed, from)
    ...
    if colison then
        for i, colType in pairs(argsColision) do
            for i, minion in ipairs(enemylib.GetMinionsInRange(range)) do
                if colType == 0 and CheckCol(unit, minion, Position, delay, radius, range, speed, from) then
                    return true
                end
            end
            for i, minion in ipairs(enemylib.GetMonstersInRange(range)) do
                if colType == 0 and CheckCol(unit, minion, Position, delay, radius, range, speed, from) then
                    return true
                end
            end
            for i, minion in ipairs(enemylib.GetEnemyHeroesInRange(range)) do
...
function isSlowed(unit, delay, speed, from)
    for i, buff in pairs(unit.buff) do
		if from and unit and buff.stacks > 0 and buff.startTime >= (delay + GetDistance(unit.pos, from) / speed) then
