...
local q = module.load("int", "Core/KogMaw/spells/q")
local w = module.load("int", "Core/KogMaw/spells/w")
local e = module.load("int", "Core/KogMaw/spells/e")
local r = module.load("int", "Core/KogMaw/spells/r")
...
core.get_action = function()
  ...
  if menu.combo.r.semi_r:get() and r.is_ready() then
  ...
  if orb.combat.is_active() then
    --if menu.combo.items.botrk:get() then
      local target = ts.get_result(function(res, obj, dist)
        if dist > 1000 or common.GetPercentHealth(target) > 40 then
          return
        end
    ...
    if menu.combo.e.use:get() == 2 then
      if e.is_ready() and e.get_prediction() then
        e.invoke_action()
        return
      end
    end
    if menu.combo.q.q:get() then
      if q.is_ready() and q.get_prediction() then
        q.invoke_action()
        return
      end
    end
    if menu.combo.w.w:get() and orb.core.can_attack() then
      if w.is_ready() and w.get_prediction() then
        w.invoke_action()
        return
      end
    end
