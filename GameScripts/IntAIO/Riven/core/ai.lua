...
local spell = module.load("int", "Core/Riven/spell/main"); --require'Core/Riven/spell/main'
local pred = module.load("int", "Core/Riven/pred/main"); --require'Core/Riven/pred/main'
...
local on_after_attack = function()
  ...
  if menu.r1:get() and orb.core.cur_attack_target and orb.core.cur_attack_target.type == TYPE_HERO then
    if pred.r1.get_action_state() then
      if e_w and menu.useEinCombo:get() then
        pred.e_w.invoke_action(true)
      elseif e and menu.useEinCombo:get() then
  ...
  if e_w and math.random()*100 <= 100 and menu.useEinCombo:get() then
    pred.e_w.invoke_action(true)
    orb.combat.set_invoke_after_attack(false)
    return true
  end
  ...
  if w then
    if e_w then
      if e_w.obj.ptr ~= w.obj.ptr and  menu.useEinCombo:get() then
        pred.e_w.invoke_action(true)
        orb.combat.set_invoke_after_attack(false)
        return true -- aa->e->w
      end
    ...
    if e then
      if e.obj.ptr ~= w.obj.ptr and menu.useEinCombo:get() then
        pred.e.invoke_action(true)
        orb.combat.set_invoke_after_attack(false)
        return true -- aa->e->w
      end
  ...
  if e and menu.useEinCombo:get() then
    pred.e.invoke_action(true)
    orb.combat.set_invoke_after_attack(false)
    return true -- aa->e->aa
  end
...
local general = function()
  ...
  if not orb.combat.target and os.clock() + network.latency > on_end_time  then
    ...
    if (menu.gap_e_w:get() or ooc < 0.75) and e_w and menu.useEinCombo:get() then
      pred.e_w.invoke_action(true)
    ...
    if (menu.gap_e_w:get() or ooc < 0.75) and e_q and menu.useEinCombo:get() then
      if not pred.w.get_spell_state() then
        pred.e_q.invoke_action(true)
        if menu.r1:get() and pred.r1.get_action_state() then
          pred.r1.invoke_action(true)
    ...
    if (menu.gap_q:get() or ooc < 0.75) and q then
      if (menu.gap_e_aa:get() or ooc < 0.75) and e then
        if menu.r1:get() and pred.r1.get_action_state() and menu.useEinCombo:get() then
          pred.e.invoke_action(true)
          pred.r1.invoke_action(true)
          return true
        end
    ...
    if (menu.gap_e_aa:get() or ooc < 0.75) and e and menu.useEinCombo:get() then
      pred.e.invoke_action(true)
      if menu.r1:get() and pred.r1.get_action_state() then
        pred.r1.invoke_action(true)
      end
...
local on_end_w = function()
  ...
  local e = pred.e.get_action_state()
  local e_q = pred.e_q.get_action_state()
  --local rh = r_hydra.get_action_state()
...
on_end_qx['Riven_Base_Q_01_detonate.troy'] = on_end_q1
on_end_qx['Riven_Base_Q_02_detonate.troy'] = on_end_q2
on_end_qx['Riven_Base_Q_03_detonate.troy'] = on_end_q3
on_end_qx['Riven_Base_Q_01_detonate_ult.troy'] = on_end_q1
on_end_qx['Riven_Base_Q_02_detonate_ult.troy'] = on_end_q2
on_end_qx['Riven_Base_Q_03_detonate_ult.troy'] = on_end_q3
