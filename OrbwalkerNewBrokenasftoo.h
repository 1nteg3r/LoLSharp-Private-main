...
public:
	...
	int BrainFarmInt = -60;
	float LaneClearWaitTimeMod = 1.2f;
	...
	void Draw()
	{
		...
		if (LasthittableMinions->Value && me->IsAlive()) {
			...
			for (auto minion : minions)
			{
				...
				auto t = AttackCastDelay() * 1050.f + 1000 * max(0, minion->Distance(me) - me->BoundingRadius()) / GetMyProjectileSpeed();
				...
				else if (dmg >= LaneClearHealthPrediction(minion, t * 3))
				{
					auto p = Engine::HpBarPos(minion);
	...
	bool ShouldWait()
	{
		...
		auto speed = (int)GetMyProjectileSpeed();
		for (auto minion : minionListAA)
		{
			if (LaneClearHealthPrediction(minion,
				(int)(Engine::GetAttackDelay() * 1000 * LaneClearWaitTimeMod) + (int)(AttackCastDelay() * 1000) + BrainFarmInt + Engine::GetPing() / 2 + 1000 * 500 / speed
			) <= CalcPhysicalDamage(me, minion, me->GetTotalAD()) * 1.2f)
				return true;
		}
	...
	{
		auto attackCalc = (int)(Engine::GetAttackDelay() * 1000 + (me->IsMelee() ? AttackCastDelay() * 1000 : AttackCastDelay() * 1000 +
			1000 * (me->GetSelfAttackRange() + 2 * me->BoundingRadius()) / global::LocalData->basicAttackMissileSpeed));

		for (auto minion : minionListAA)
		{
			if ((noneKillableMinion != nullptr ? noneKillableMinion->NetworkID() != minion->NetworkID() : true) && LaneClearHealthPrediction(minion, attackCalc) <= CalcPhysicalDamage(me, minion, me->GetTotalAD()))
				return true;
		}
		return false;
	}
	...
	float LaneClearHealthPrediction(CObject* unit, int delta, int delay = 0)
	{
		auto predictedDamage = 0.f;
		std::unordered_map<DWORD, ActiveAttackstruct>::iterator it;
		for (it = ActiveAttacks.begin(); it != ActiveAttacks.end(); it++)
		{
			auto attack = it->second;
			if (attack.target == unit->Index() && Engine::GameTimeTickCount() - 100 <= attack.startTime + attack.animationTime)
			{
				auto n = 1;
				auto fromT = attack.startTime;
				auto toT = Engine::GameTimeTickCount() + delta;
				auto source = (CObject*)attack.source;

				while (fromT < toT)
				{
					auto travelTime = fromT + attack.Delay + 1000.f * max(0, unit->Distance(source) - source->BoundingRadius()) / attack.projectileSpeed + 10.f;
	...
	CObject* GetTarget()
	{
		...
		minionListAA.clear();
		for (auto minion : Engine::GetMinionsAround(me->GetSelfAttackRange() + 300.f, 1))
		{
		...
		if (mode & ScriptMode::LaneClear || mode & ScriptMode::Harass || mode & ScriptMode::LastHit)
		{
			...
			{
				auto t = firstT + 1000 * (int)max(0, me->ServerPosition().Distance(minion->ServerPosition()) - me->BoundingRadius()) / projectileSpeed;

				if (!PushPriority->Value)
				{
					t += 200 + Engine::GetPing() / 2;
				}
				...
				else
				{
					...
					if (predHealth <= 0)
					{
						if (GetHealthPrediction(minion, t - 50) > 0)
						{
		...
		//			if (it_turretMinion != minions.end())
		//			{
		...
		//				float turretLandTick = turretStarTick + (TowerAttackCastDelay * 1000) + //closestTower.AttackCastDelay = 1500
		//					1000 *
		//					max(
		//						0,
		...
		//					i < turretLandTick + 10 * TowerAttackDelay * 1000 + 50;
		//					i = i + TowerAttackDelay * 1000)
		//				{
		//					float time = i - Engine::GameTimeTickCount() + Engine::GetPing() / 2;
		//					float predHP =
		...
		//						(turretAttackCount + 1) *
		//						(TowerAttackDelay * 1000) -
		//						Engine::GameTimeTickCount();

		//					float timeUntilAttackReady = LastAATick() + (Engine::GetAttackDelay() * 1000) >
		//						Engine::GameTimeTickCount() + Engine::GetPing() / 2 + 25
		//						? LastAATick() + (Engine::GetAttackDelay() * 1000) -
		//						(Engine::GameTimeTickCount() + Engine::GetPing() / 2 + 25)
		//						: 0;
		...
		//						: AttackCastDelay() * 1000 +
		//						1000 * max(0, turretMinion->Distance(me) - me->BoundingRadius()) /
		//						global::LocalData->basicAttackMissileSpeed;

		//					if (hits >= 1 &&
		//						hits * Engine::GetAttackDelay() * 1000 + timeUntilAttackReady + timeToLandAttack <
		//						timeBeforeDie)
		//					{
		...
		//							(turretAttackCount * TowerAttackDelay / Engine::GetAttackDelay()) *
		//							CalcPhysicalDamage(me, lastminion, me->GetTotalAD()) > lastminion->Health())
		//						{
		//							return lastminion;
		//						}
		//						if (minions.size() >= 5 && 1.f / Engine::GetAttackDelay() >= 1.2f)
		//						{
		//							return lastminion;
		//						}
		//					}
		...
		if (mode & ScriptMode::LaneClear)
		{
			if (!ShouldWait())
			{
				auto firstT2 = (int)(Engine::GetAttackDelay() * 1000 * LaneClearWaitTimeMod) + (int)(AttackCastDelay() * 1000) + BrainFarmInt + Engine::GetPing() / 2;
				std::vector<CObject*> laneclearminion_list = minionListAA;
				...
				{
					auto t = firstT2 + 1000 * (int)max(0, me->ServerPosition().Distance(minion->ServerPosition()) - me->BoundingRadius()) / projectileSpeed;

					float predHealth = LaneClearHealthPrediction(minion, t);
